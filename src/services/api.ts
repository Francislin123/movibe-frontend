...
export const getHealth = () =>
  unwrap<HealthResponse>(client.get('/health'))
...
export const getEvents = () =>
  unwrap<EventResponse[]>(client.get('/events'))
...
export const getUsers = () =>
  unwrap<UserResponse[]>(client.get('/users'))
...
function normalise(err: unknown): ApiError {
  if (err instanceof AxiosError) {
    ...
    if (status === 0 || !err.response)
      return { status: 0, message: 'Não foi possível conectar ao servidor. Verifique se o backend está rodando na porta 8080.' }
    if (status === 404)
      return { status: 404, message: 'Recurso não encontrado (404).' }
    if (status === 400)
      return { status: 400, message: payload?.error ?? payload?.message ?? 'Requisição inválida (400).' }
    if (status === 413)
      return { status: 413, message: 'Arquivo muito grande (413).' }

    return { status, message: payload?.error ?? payload?.message ?? `Erro inesperado (${status}).` }
  }
